...
public class DictManagerOld {
    ...
    Context context;
    static DictManagerOld manager;

    public static DictManagerOld getInstance(Context context) {
        if (manager == null) {
            manager = new DictManagerOld(context);
        }
        return manager;
    }
     ...
     *
     */
    public DictManagerOld(Context context) {
    ...
    /**
     *
     * Add a new dictionary. Storing the book name and book name's '.ifo' file path.
    ...
    public void loadDictionary(String mBookName, File idxFile, Handler handler) {
        ...
        new LoadDictionary(mBookName, idxFile, helper, handler).start();
    }
    ...
    public void clearAllDictionaries() {
        ...
        if (file != null && context.deleteDatabase(file.getAbsolutePath() + File.separator
                + DictSQLiteDefine.DATABASE_NAME)) {
